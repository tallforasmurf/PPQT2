...
Externally-available functions, some from QTextDocument, some unique:
    ...
    all_blocks()         iterator returning each QTextBlock in turn, first to last.
    ...
    line_start(a)        character offset in the document to the start of line
                         number a, used by the translator.
...
class Document(QTextDocument):
    ...
    Under the PyQt5 (QString-less) API, a call to self.toPlainText gets a
    Python string which has to have been mem-copied from a QString -- and not
    simply copied, either, because the \u2029 line delimiters that Qt uses,
    are converted to \n characters! In a larger book that could be quite a
    slow operation. So we minimize the number of such copies by caching,
    returning the same string as long as it is valid. When the user makes any
    edit change to the document we get a signal and clear self.text.
    '''
    ...
    contains at least 1 QTextBlock. Hence self.blockCount() >= 1 and 
    self.begin() != self.end()
    
    Note B, self.findBlockByNumber(x)==self.end() for any x that is invalid,
    ...
    i.e. 0 <= a < self.blockCount, 0 <= z < self.blockCount, a <= z.
    
    The initial tests guarantee that 0 <= a < .blockCount and a <= z,
    but we don't check that z < .blockCount; if z did exceed .blockcount,
    we will reach the case of tbx == self.end() and stop.
    
    Hence the loop can start with a yield, but it needs two termination
    ...
    def a_to_z_blocks(self, a, z):
        ...
        if a <= z and tb.isValid() :
            while True :
                ...
                if tb == tbx : break # done
                tb = tb.next()
                if not tb.isValid() : break # off the end
    '''
    ...
    def all_lines(self):
        ...
        while tb.isValid():
            ...
            tb = tb.next()
    '''
    2. A range of lines from LINE NUMBER, not text block number, a to z
    inclusive, guarding against invalid ranges and a completely empty doc.
    Line #s are origin-1, textblock #s origin-0. Same comments as for
    a_to_z_blocks.
    '''
    ...
    def z_to_a_lines(self, a, z):
        ...
        if not tb.isValid() : # create a valid line number z
            tb = self.lastBlock()
            z = tb.blockNumber() + 1
        ...
        while True:
            ...
            tb = tb.previous()
    '''
    4. A range of lines included in the selection of a text cursor. Includes
    the first and last lines even if the selection does not completely cover
    them.
    
    Note that a cursor's position and anchor, the two character indexes that
    define its selection, can be in either sequence. So we find the lower and
    higher character indexes, get the numbers of the text blocks that contain
    those points (possibly the same text block), and defer to out
    a_to_z_lines method.
    '''
