...
and user switch choices are saved with the book. These operations use the
...
'''
Global function to compile an RE with the flags we apply to all user regexes.
If the compile throws an error the caller should catch it. case_switch is an
instance of QCheckBox, the Respect Case switch.
'''
...
The remember() method stores a new string in the list. It is called from the\
associated FindRepEdit, or from the parent FindPanel when loading strings\
from the metadata.
...
class RecallMenuButton(QToolButton):
    ...
    ''' Clear the stack of stored actions, used from metadata-reader to
    ...
    delete it so it effectively moves to the top.
    '''
    ...
    '''
    A menu item has been chosen. The menu action is passed with the signal;
    set the text of that action in our partner. Note in the partner that the
    string is user-entered, so it will be remembered.
    '''
...
as to be visible and useful to a regex. (Note that a non-regex search cannot
match to a newline anyway; this is a limitation of the find function of
QTextDocument.) If the regex switch is off, the search for "\n" will fail as
it should, unless by chance there are those two characters in the text.
...
regex the background of the field is turned pink.

Note that Qt offers a QRegularExpressionValidator class, but we can't be sure
that its definition of regex syntax fully matches that of the regex module
that we use. Instead we validate by running a regex compile.

Regex validation is disabled for the Replace edits, as there is no "compile"
for a regex replace string, so no simple way to know if it is syntactically
valid.

The object knows an associated RecallMenuButton (passed to the constructor)
and provides a note_user() method which is called (e.g. by the Next button)
when the field contents are user-entered and should be saved for recall.

First is the custom QValidator which performs a single functions: fixing a
...
class FindRepEdit(QLineEdit):
    def __init__(self, my_recall, ref_to_find, sw_regex, sw_respect_case, parent=None):
        ...
        # create and establish our validator
    ...
    text from a menu or user button. So if you load this field by selecting a
    prior string from the recall button, and don't edit it, it will not be
    user-edited and if used, will not be pushed on the recall stack. But if
    you edit the recalled string in any way, that string will be saved.
    '''
    ...
    def set_background_pink(self,diagnostic):
        ...
        palette.setColor( QPalette.ColorGroup.Normal,
                          QPalette.ColorRole.Base, QColor('Pink') )
    ...
    def set_background_white(self):
        palette = self.palette()
        palette.setColor( QPalette.ColorGroup.Normal,
                          QPalette.ColorRole.Base, QColor('White') )
        self.setPalette(palette)
        self.setToolTip('')
    ...
    the validator whenever the text changes (we want to be sure the validator
    is finished before we do this work, so it isn't called directly for our
    own textChanged signal). It is also called by the toggled signal from
    either sw_regex or sw_respect_case.
    
    Note that when regex.compile raises an error, the str() value of the
    error is a diagnostic. We put that in our tooltip, so the user can
    determine the problem by hovering the mouse over the text.
    '''
    def check_regex(self):
        if self.sw_regex.isChecked() and self.find_ref is None :
            ...
            try:
                ...
                self.set_background_white() # apparently it's good
...
'''
       Class of the user-programmable push buttons.

Each button can store the values of the fields of the upper part of the panel,
and reload the panel to those values when desired.

Field values are stored in the form of a Python dict with these keys:
...
The button constructor takes the __repr__ string of a dict as its argument
and converts it to a dict, stored as self.udict. See .load_dict() for this process.

The FindPanel (below) method user_button_output() saves button values to a
file by accessessing self.udict and writing its __repr__ value to a file.
FindPanel method user_button_input() reads such a representation from a file
and loads the button.

When the user clicks a button, the normal QPushButton.clicked signal is
passed up to FindPanel method user_button_click(). There the dict values
stored in the clicked button are used to set the fields of the panel.

When the button is right-clicked (Mac: ctl-clicked), we generate the
user_button_ctl_click signal. This is also handled in the FindPanel, where
the user is queried for a new label for the button, and the current panel
widget values are saved in this button's dict.
...
class UserButton(QPushButton):
    ...
    ''' prototype for a new info dictionary '''
    ...
    ''' dictionary of the expected/required data types for a dict '''
    ...
    def __init__(self, init_dict=None, parent=None):
        ...
        self.setContextMenuPolicy(Qt.ContextMenuPolicy.PreventContextMenu) # we handle right-click
        if init_dict is None :
            # no dict given, probably initializing in a new Book.
            # install the empty model dict as a default.
            self.udict = UserButton.model_dict.copy()
            self.setText(self.udict['label'])
            self.setToolTip('')
        else :
            # a dict was passed, use it to initialize
    ...
    '''
    Trap a right-click or control-click and pass it as a signal to the parent
    FindPanel. The FindPanel will query the user for a new button label if
    one is given, will load our dict with the values of the present find
    fields.
    ...
    def mouseReleaseEvent(self,event):
        if 2 != int(event.button()) : # right- or control-click
            ...
            # regular click, pass it up to FindPanel
            event.accept()
            self.user_button_ctl_click.emit()

    '''
    Subroutine to load this button's values from a Python string that
    purports to be the Python __repr__ of a dictionary.
    
    This is called during initialization, or when loading user Buttons from a
    file or metadata. Since the dict string might be user-edited, we treat it
    with suspicion. We require it to be a valid Python literal form of a dict
    type. It must have an entry 'label':'string' with a reasonable length of
    'string'. Other entries are optional but each key must exist in our model
    dict and have the expected type for that key. If any test fails we leave
    our existing dict alone.
    '''
    def  load_dict(self,dictrepr):
        try:
            ...
            Note the compiler in ast chokes on literal tabs, so replace them
            with spaces.
            '''
            err = 'Not a valid dict literal'
            literal = ast.literal_eval(dictrepr.replace(u'\t',u' '))
            '''
            Check the dict's keys against model_dict. If in fact it isn't a
            dict, the .keys() call raises an error.
            '''
            ...
            if 'label' not in literal :
                ...
                raise ValueError
            '''
            All is good, go ahead and use it, including setting its label
            as our button text and its tooltip value as ours.
            '''
            self.udict = ok_dict
            ...
            ''' some error raised, report it in the log only '''
...
A Find Panel is created, along with the edit and other UI tabs,
by the Book when it is initializing.

From the Book we get access to the EditView and EditModel objects for our
book, and the metadata manager where we load/save user button values when
...
class FindPanel(QWidget):
    ...
    def __init__(self, my_book, parent=None):
        super().__init__(parent)
        # save access to the book and to the edit view and edit model.
        self.book = my_book
        ...
        '''
        Lay our UI out -- the boring code is at the end of the module
        where it creates and lays out the objects:
            Checkboxes,
        ...
        self.user_buttons = []
        self._uic() # Out of line, make all the above.
        '''
        Now hook up signals from the switches and buttons to the code
        ...
        '''
        Connect the four do-search buttons to the common do_search method,
        passing a 2-bit code to distinguish each.
        '''
        self.find_first.clicked.connect(
            lambda : self.start_search(
                FindPanel.SEARCH_LIMIT)
            )
        self.find_last.clicked.connect(
            lambda : self.start_search(
                FindPanel.SEARCH_BACKWARD+FindPanel.SEARCH_LIMIT)
            )
        self.find_prior.clicked.connect(
            lambda : self.start_search
            (FindPanel.SEARCH_BACKWARD)
            )
        self.find_next.clicked.connect(
            lambda : self.start_search(0)
        )
        ''' Also connect find_field's returnPressed to do a "next".'''
        self.find_field.returnPressed.connect(
            lambda : self.start_search(0)
        )
        ...
        '''
        Connect the edit view's selectionChanged signal to a slot where we
        note the selection is no longer our work. Whenever the user does
        anything in the edit window that changes the selection, control will
        pass through edit_selection_change().
        '''
        self.editv.get_actual_editor().selectionChanged.connect( self.edit_selection_change )
        '''
        Connect the edit view's editFindKey signal to our edit_key_press method.
        '''
    ...
    changes. If one is turning "on" turn the other one "off".
    '''
    ...
    '''
    Come here when the state of the Regex switch changes. Disable or enable
    the Whole Word switch because Whole Word doesn't apply to regex search.
    '''
    ...
    modifiers and the event key value. Possible values were set up in
    constants.py as a key signature for quick lookup.
    '''
    def edit_key_press(self,kkey):
        if  kkey == C.CTL_F or kkey == C.CTL_SHFT_F :
            '''
            ^f means, move the focus to the Find panel.
            ^F means, that and use the current selection as the find string.
            
            Make sure our panel is visible, focus in Find text, with existing
            text selected for easy replacement.
            '''
    ...
    '''
    Methods related to setting the valid range for searching.
    
    The current range within which searching is done is controlled by the In
    Selection switch. When that is off, the selection is the whole document.

    When In Selection goes False->True, we query the current QTextCursor held
    by the editor. If its related selection is large enough, that becomes the
    range for all find/replace work.
    
    Control comes here when the state of the In Selection switch changes.
    '''
    def insel_change(self,state):
        if not state :
            ''' Switch went on-to-off; clear range '''
            self.editv.clear_find_range()
            return
        '''
        Switch changed from off to on. Get the current selection and check
        that it spans at least 100 characters and 4 lines. These are
        arbitrary limits.
        '''
        tc = self.editv.get_cursor() # copy of edit cursor
        a = tc.selectionStart()
        z = tc.selectionEnd()
        ba = self.editm.findBlock(a) # QTextBlock containing a
        bz = self.editm.findBlock(z) # ..and z
        if (100 > (z-a)) and ( 4 > ( bz.blockNumber() - ba.blockNumber() ) ):
            ...
            '''
            Flip the In Selection switch back to "off" state. That generates
            a recursive call to this method
            '''
            self.sw_in_range.setChecked(False)
            return
        self.editv.set_find_range() # range was ok, set it
    ...
    The search is started and the Find panel is left with the focus so
    the user can continue searching from that point.
    '''
    def find_this(self, what, case=False, word=False, regex=False, repl=None):
        self.find_field.setText(what)
        self.find_field.note_user(False) # don't push "what" to recall list
    ...
    Prepare to perform a click of any one of the four do-search buttons.
    
    flag argument has SEARCH_LIMIT for First and Last and SEARCH_BACKWARD for
    Last and Prior. Here we choose the correct starting position and call
    real_search to execute one find.
    
    We choose starting positions so as to NOT find overlapping matches. In
    choosing the start position we assume a find-range has been set, but if
    it has not, the range cursor selects the whole document anyway.
    
    When a match is found, call editv.center_this() to ensure the found text
    is visible. If no match, beep and leave the edit cursor unchanged.
    '''
    ...
    def start_search(self, flag):
        ...
        if flag & FindPanel.SEARCH_LIMIT :
            ''' Doing Find-First or -Last, so start at the limit of the range. '''
            ...
            '''
            Doing Find-Next or -Prior, start from one end of the current
        ...
        start_tc.setPosition(pos) # clears any selection in start_tc
        ''' Look for a match, forward or backward from start_tc. '''
        find_tc = self.real_search( start_tc, flag & FindPanel.SEARCH_BACKWARD )
        '''
        If find_tc now has a selection, there was a match. However it could
    ...
    for a single find, or from replace_all below to find each possible search
    target. start_tc is a cursor marking the starting position, and flag is
    zero or SEARCH_BACKWARD. Also depends on the whole-word, respect-case,
    and regex switches.
    '''
    def real_search(self, start_tc, flag):
        ...
        if not self.sw_regex.isChecked() :
            ...
            '''
            Perform a regex search. When this was written QTextDocument
            didn't support full PCREs or had some other shortcoming that made
            it advisable to use the regex package instead. Current Qt does
            support full PCREs and Unicode, for both forward and backward
            searches (https://doc.qt.io/qt-6/qtextdocument.html#find-2) It
            still does not support the fuzzy matches and levenstein distance
            matches that regex does; but it is not clear that these are
            essential to practical use of PPQT.
            
            At this point to rewrite to use Qt regex search directly would
            also mean changing how the validator does on-the-fly checking of
            regex string in the find text field also. So -- TODO think about
            that.
            '''
            find_tc = start_tc # assume failure to match
            ...
            if re : # is not None, the find text is a valid regex
                ...
                if flag & FindPanel.SEARCH_BACKWARD :
                    if fp.reverse is None :
                        '''
                        The current find re was compiled for forward. Compile
                        a version for reverse, and save it in case we do this
                        again
                        '''
                ...
                At this point we get a copy of the whole document as a string(!)
                and do the search within that string.
                '''
                text = self.editm.full_text() # yes, copy of the whole document
                if flag & FindPanel.SEARCH_BACKWARD :
                ...
                if fp.match :
                    '''
                    We have a match, create a QTextCursor to reference the
                    matched string within the QTextDocument.
                    '''
                    (a, z) = fp.match.span()
    ...
    '''
    Execute one of the three do-replace buttons. Get a reference to which of
    the three was clicked using QObject.sender(). Convert that to an index in
    our list of replace buttons. Sample the current state of the and-next,
    and-prior and do-all checkboxes. Pass those 4 items to start_replace.
    '''
    ...
    particular values for the switches and-next, and-prior and do_all. This
    method can be called from replace_click above, or directly via a
    keystroke event. The key action for ctl-t passes True for andnext, while
    shift-ctl-t passes True for andprior.
    '''
    
    def start_replace(self, button, and_next, and_prior, do_all ):
        if do_all :
            ''' Global-replace is a whole different thing. Go do it. '''
            self.do_global_replace(button)
            return
        '''
        Single replace: If the current selection is not the result of
        a preceding Find, we should not replaceit. Beep and do nothing.
        '''
        if not self.selection_by_find :
            utilities.beep()
            return
        ''' Proceed with single replace of current selection. '''
        tc = self.editv.get_cursor() # access to current selection
        pa = tc.selectionStart() # save index of start of selection
        rb = self.replace_fields[button] # quick ref to the button object
        rb.content_used() # push current value on the recall list
        if not self.sw_regex.isChecked() or (self.find_field.match is None) :
            '''
            Regex-style replacement is not requested, so just replace the
            current selection with the contents of the rep text field.
            '''
            tc.insertText(rb.text())
        else :
            '''
            Current selection was created by a Find action, presumably using
            our current regex. Since the selection hasn't changed, the match
            object in find_field is still valid, so use it to generate the
            replacement string.
            
            This is where we find out about a mistake in the Replacement
            string syntax. Two different exceptions can occur, a regex.error
            for syntax, or an IndexError for a nonexistent group reference.
            The except clause doesn't allow naming more than one exception
            type, and we don't want to trap any other exception types so we
            have two identical except clauses.
            '''
            try:
                ...
                return
        '''
        Either of the preceding insertText() calls left the cursor without a
        selection and positioned at the end of the insert. Reselect back to
        the start so the inserted text is selected. Besides being convenient,
        this properly positions the cursor in case we are doing
        replace-and_prior.
    ...
    '''
    Execute global replace, that is, replace all matching strings in the
    current search range (which may be the whole book).
    
    We are going to take advantage of the regex.finditer(string,pos,endpos)
    method to generate all matches. If the current search string is not
    already a regex, make it usable as one by escaping all regex magic
    characters in the find pattern. Ditto for the replace string, but we only
    escape backslashes, because only "\#" and "\g<#>" are magic in regex
    replace strings.
    
    Collect all the match objects in the search range in a list. (We have to
    collect them all before doing the replace because we need to show the
    user the count of matches.)
    
    When the user says go, we run backward through the list (from end of
    document toward the top) converting each match to a cursor, and replacing
    the selected text with the expanded match. This is all done under an undo
    macro so it is a single undo.
    '''
    def do_global_replace(self, button):
        ...
        info_with = f_pattern + _TR(
            ...
            " with " ) + r_pattern
        if self.sw_regex.isChecked() :
            ''' supposedly a regex find, make sure it is a valid one '''
            rex = self.find_field.regex
            if rex is None : # bad regex syntax
                utilities.beep()
                return
        else :
            ''' Find string does not claim to be a regex pattern, make it one. '''
    ...
    Come here when any UserButton is clicked. The argument is a boolean that
    we don't care about. All user buttons connect here; we find out which one
    with the sender() method.
    
    If the button has defined contents, move its dict values into our actual
    widgets. Some dict items may be None, meaning, no change.
    
    These values will NOT go in the recall stack for their
    find/rep field. (The .setText method clears the note_user flag.)
    '''
    ...
    '''
    Come here when any UserButton gets a control-click. Prompt the user for a
    new button label, and if one is given, load our current widget values
    ...
    The mainwindow manages the File > Load/Save Find Buttons including
    preparation of UTF-8 file streams.
    '''
...
'''
    ...
    buttons, it is a dict { "n" : [str9, str8... ],...} for "n" in "0123" and
    the strings being the recall stack strings. The strings are listed in
    reverse order so that when they are read back, they are pushed onto the
    recall stack in the correct order.
    '''
    ...
    checks because the user might have edited the .meta file.
    '''
    ...
    '''
    To read back the user button metadata we look for stupid errors in the
    top-level value. However, the user button itself has a detailed
    error-checker for the string repr of a dict, so we convert the dict to a
    string and let the button do the checking.
    '''
    ...
    '''
    Here, out of line from __init__(), we make all the widgets and lay them
    out. Signals are connected later on in __init__. Translate the button
    titles and tooltips.
    '''
    def _uic(self):
        '''
        To put widgets in a frame, the frame must be the parent of the layout
        that contains them. This frame is for the find controls, 4
